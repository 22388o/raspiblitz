...
# see https://github.com/ElementsProject/elements/releases
...
if [ $# -eq 0 ] || [ "$1" = "-h" ] || [ "$1" = "-help" ]; then
 ...
 echo "/home/admin/config.scripts/bonus.elements.sh on|off|menu"
...
# show info menu
if [ "$1" = "menu" ]; then
  dialog --title " Elements ${pinnedVersion} " --msgbox "\n
ELements is a command line tool to rescue locked funds.\n\n
...
if [ "$1" = "1" ] || [ "$1" = "on" ]; then
  ...
  echo 
  echo "# Elements v${pinnedVersion} for ${OSversion}"
  ...
  if [ "${binaryChecksum}" != "${SHA256}" ]; then
    echo "# !!! FAIL !!! Downloaded Elements BINARY not matching SHA256 checksum: ${SHA256}"
    exit 1
  fi
  ...
  if [ ${fingerprint} -lt 1 ]; then
    echo
    echo "# !!! BUILD WARNING --> Elements PGP author not as expected"
  ...
  sudo install -m 0755 -o bitcoin -g bitcoin -t /home/bitcoin/bin elements-linux-${OSversion}-v${pinnedVersion}/*
  sleep 3
  installed=$(sudo -u bitcoin /home/bitcoin/bin/elements --version)
  ...
  echo "
# Elements is a command line tool.
...
if [ "$1" = "0" ] || [ "$1" = "off" ]; then
  ...
  # setting value in raspi blitz config
  sudo sed -i "s/^chantools=.*/elements=off/g" /mnt/hdd/raspiblitz.conf
  
  echo "# REMOVING Elemens"
  sudo rm -rf /home/admin/download/elements*
  sudo rm -rf /home/bitcoin/bin/elements*
  echo "# OK, elements is removed."
  exit 0
